...
class Tile:
    ...
    def get_dist(self, coord: Tuple[int, int]) -> int:
        return max(
            abs(self.coordinate[0] - coord[0]), abs(self.coordinate[1] - coord[1])
        )
...
class TileGrid:
    def __init__(self) -> None:
        ...
        for locations, resource in [
            (BERRY_LOCATIONS, Berry),
            (TREE_LOCATIONS, Tree),
            (ROCK_LOCATIONS, Rocks),
            (VEIN_LOCATIONS, Vein),
        ]:
            for left, top, width, height in locations:
                for i, j in itertools.product(
                    range(left, left + width), range(top, top + height)
                ):
